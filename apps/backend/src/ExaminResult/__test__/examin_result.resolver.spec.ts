...
describe('開始測試 Examin_Result', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ExaminResultResolver,
        { provide: ExaminResultService, useValue: resultService },
  ...
  describe('測試刪除結果', () => {
    it('應該正常刪除', async () => {
      const result = await resolver.removeExaminResult(10);
    ...
    it('應該無法刪除', async () => {
      ...
      expect(resultService.remove).toHaveBeenCalledWith(999);
